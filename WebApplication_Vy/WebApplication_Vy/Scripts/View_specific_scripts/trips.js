...
trips = fetch("https://booking.cloud.nsb.no/api/itineraries/search", {
    ...
    $.each(itineraries, function (i, value) {
        ...
        var priceDetails = "<div class='col m-3'>\
                                        <div class='row'>\
                                            <div class='w-100 text-center'><button type='button' class='price-btn p-3 btn btn-primary'>Show price details</button></div >\
                                        </div>\
                                        <div style='display: none;'>"
        $.each(people, function (p, ammount) {
            for (var v = 0; v < ammount; v++) {
                console.log(p);
                if (p === "Adult") {
                    priceDetails += "<div class='row'>\
                                                <div class='col text-center ml-2 mr-2'>" + p + "  -  " + originalPrice + ",-</div>\
                                            </div>"
                    price += originalPrice;
